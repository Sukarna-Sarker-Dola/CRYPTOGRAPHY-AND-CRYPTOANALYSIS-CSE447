...
Original file is located at
    https://colab.research.google.com/drive/1RrP1NTRBUK4HZGEKNOZjJkrqc2aX28ka
...
message = "After fighting off the alligator, Brian still had to face the anaconda" #this is the message 

message=message.encode() #encoding the message
print(message) #encided version of message
key = b'\xa1\xfcw"?3\x91\x1c\t\x9c\x91\xe2He\x935' #took a random 16 bit key
print(key)
c_obj = cmac.CMAC(algorithms.TripleDES(key)) #cmac er object return korbe jeta diye message updTate korbi

c_obj.update(message) #message info is in c obj

signature = c_obj.finalize() #generating signature

print (f"Message: {message.decode()}" )

print (f"CMAC signature: {binascii.b2a_hex(signature).decode()}") #decode er signature
